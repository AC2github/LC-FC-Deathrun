...
include("menu/vgui/cl_jukebox.lua")
...
include("XPM/sh_jukebox.lua")

-- XPM (CL)
include("XPM/client/cl_networking.lua")
include("XPM/client/cl_jukebox.lua")
...
function GM:HUDPaint( )
	...
	local var = math.abs( math.sin( CurTime() * 2 ) )
	local hudsize = math.Approach( 75, 100, var * 20 )
	
	local max_x = math.Clamp(hudsize, 75, 100)
	local max_y = math.Clamp(hudsize, 75-25, 93)
	
	local textsize = math.Approach( .75, 1, var * .2 )
	
	surface.SetDrawColor(Color(255,255,255,255))
	surface.DrawTexturedRect(85 - (max_x / 2), (ScrH() - 75) - (max_y / 2), max_x, max_y)
	...
	if (jukebox.IsPlaying) and (jukebox.CurrentSong != "") then
		draw.AAText("Playing: "..jukebox.CurrentSong, "Deathrun_SmoothMed", ScrW() - 5, 2, color_white, TEXT_ALIGN_RIGHT)
		draw.AAText( string.ToMinutesSeconds(jukebox.CurrentTime).." / "..(string.ToMinutesSeconds(jukebox.GetTimeFromSong(jukebox.CurrentSong))), "Deathrun_SmoothMed", ScrW() - 5, 25, color_white, TEXT_ALIGN_RIGHT)
	end
	
	--drawSpecialText(health, 70 - (textsize / 2), (ScrH() - 90) - (textsize / 2), textsize, textsize, Angle( 0, 90, 0 ))	
	draw.SimpleText(health, "deathrun_health", 85, ScrH() - 80, color_white, TEXT_ALIGN_CENTER)
	
	surface.SetDrawColor(Color(0, 0, 0, 200))
	surface.SetMaterial(gradient)
	surface.DrawTexturedRect(-21.25, (ScrH() - 120) - (max_y / 2), 200, 40)
	draw.SimpleText(rt, "deathrun_health", 85, (ScrH() - 120) - (max_y / 2), color_white, TEXT_ALIGN_CENTER)
	
	if (LocalPlayer().PlayerData) then
		-- Level
		surface.SetDrawColor(Color(62, 62, 62, 200))
		surface.DrawOutlinedRect(15, 15, 50, 30)
		surface.DrawOutlinedRect(245, 15, 50, 30)
		
		draw.RoundedBox(0, 15, 15, 50, 30, Color(0, 0, 0, 200))
		draw.RoundedBox(0, 245, 15, 50, 30, Color(0, 0, 0, 200))
		draw.RoundedBox(0, 70, 15, 170, 30, Color(0, 0, 0, 200))
		
		draw.AAText("Lvl "..LocalPlayer().PlayerData["level"], "Deathrun_Smooth", 25 + 13.5, 24, color_white, TEXT_ALIGN_CENTER)
		draw.AAText("Lvl "..(LocalPlayer().PlayerData["level"] + 1), "Deathrun_Smooth", 255.5 + 13.5, 24, color_white, TEXT_ALIGN_CENTER)
		
		local size = (170 * LocalPlayer().PlayerData["xp"]) / XPM.levels[LocalPlayer().PlayerData["level"]+1].xp
		draw.RoundedBox(0, 70, 15, size, 30, Color(32, 124, 150, 220))
		
		surface.SetDrawColor(Color(62, 62, 62, 200))
		surface.DrawOutlinedRect(70, 15, 170, 30)
		...
		if (LocalPlayer():GetNWInt("timeplayed")) then
			...
			if trace.Entity and trace.Entity:IsValid() and trace.Entity:IsPlayer() then
				if LocalPlayer():Alive() then
					draw.AAText(trace.Entity:Name().." time played: "..timeToStr(math.Round(trace.Entity:GetNWInt("timeplayed") + CurTime())), "Deathrun_Smooth", 17, 110, color_white, TEXT_ALIGN_LEFT)
				else
			...
			if (LocalPlayer():Alive()) then
				draw.AAText("Time played: "..timeToStr(math.Round(LocalPlayer():GetNWInt("timeplayed") + CurTime())), "Deathrun_Smooth", 17, 90, color_white, TEXT_ALIGN_LEFT)
			else
	...
	if (LocalPlayer():Alive()) then
		-- Velocity
		surface.DrawOutlinedRect(70, 55, 225, 30)
		draw.RoundedBox(0, 70, 55, 225, 30, Color(0, 0, 0, 200))
		
		draw.RoundedBox(0, 15, 55, 50, 30, Color(0, 0, 0, 200))
		
		draw.AAText("Speed", "HudHintTextSmall", 25 + 13.5, 65, color_white, TEXT_ALIGN_CENTER)
		
		local size = (170 * LocalPlayer():GetVelocity():Length()) / 1000
		draw.RoundedBox(0, 70, 55, math.Clamp(size, 0, 1000), 30, Color(32, 124, 150, 220))
	end
...
function GM:GetScoreboardNameColor( ply )
	...
	-- Sanne, stupid bet...
	if ply:SteamID() == "STEAM_0:0:51692222" then
		i = i + 1
	...
	-- Amy
	if ply:SteamID() == "STEAM_0:1:65852621" then return Color(0, 0, 255, 255) end
	-- AC
	if ply:SteamID() == "STEAM_0:0:16134549" then return Color(math.random(1, 255), math.random(1, 255), math.random(1, 255), 255) end
	-- Sherloch
	if ply:SteamID() ==  "STEAM_0:0:54089189" then return Color(255, 255, 0, 255) end
	-- Matt
	if ply:SteamID() == "STEAM_0:0:44498988" then return Color(127, 255, 0, 255) end
	-- Pixieee
	if ply:SteamID() == "STEAM_0:0:9987939" then return Color(0, 255, 0, 255) end
	-- Rodders
	if ply:SteamID() == "STEAM_0:0:57700030" then return Color(0, 0, 0) end
	-- Peanut
	if ply:SteamID() == "STEAM_0:0:25063465" then return Color(255, 0, 0) end
...
local function ShowHelp()
	...
	MainFrame:AddButton("Credits", "icon16/eye.png", function() MainFrame:RemoveActivePanel() CreateDeathrunHelp("Credits", GAMEMODE.CreditsText) end)
		...
		for k,v in pairs(SortedItems) do
			...
			perks[k].Paint = function(self)
				...
				elseif (LocalPlayer().PlayerData["level"] >= XPM.GetItemData(v).level) then
					...
					draw.RoundedBox(0, 0, 0, self:GetWide(), self:GetTall(), Color(149, 14, 14, 155))
				end

				draw.SimpleText(XPM.GetItemData(v).name .. " (LvL "..XPM.GetItemData(v).level..")", "Deathrun_SmoothMed", 5,2, Color(192, 192, 192, 255), TEXT_ALIGN_LEFT)
	...
	MainFrame:AddButton("Jukebox", "icon16/control_play.png", function() MainFrame:RemoveActivePanel()
	--if (LocalPlayer():IsAdmin()) then
			jukebox.Menu = vgui.Create("DeathrunJukebox")
			
			jukebox.Menu.NewThink = function()
				if not (MainFrame:IsValid()) then
					jukebox.Menu:Remove()
				end
			end
			--LocalPlayer:ChatPrint("The jukebox has been disabled.")
		--else
		--	CreateDeathrunHelp("Help", "Coming soon")
		--end
	end)
